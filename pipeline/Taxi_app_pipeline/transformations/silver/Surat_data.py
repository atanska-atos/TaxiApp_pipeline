...
def surat_all_data():
    df = spark.read.table(table_name).filter(expr("City_Id == 'Surat'")).select("Trip_Id","Date","Passenger_Type","Distance_Travelled","Fare_Amount","Passenger_Rating","Driver_Rating")
    return df