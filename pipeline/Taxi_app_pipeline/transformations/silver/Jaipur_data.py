...
@dp.table(
    name = "transportation_app.silver.Jaipur_trip",
...
def jaipur_all_data():
    ...
    df = trips_df.join(city_df, trips_df.City_Id == city_df.City_Id).where(col("City_Name") == "Jaipur").select("Trip_Id","Date","Passenger_Type","Distance_Travelled","Fare_Amount","Passenger_Rating","Driver_Rating")
    return df