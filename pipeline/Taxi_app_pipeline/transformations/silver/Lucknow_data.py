...
@dp.table(
    name = "transportation_app.silver.Lucknow_trip",
...
def lucknow_all_data():
    city_df = spark.read.table(city_table)
    trips_df = spark.read.table(table_name)
    df = trips_df.join(city_df, trips_df.City_Id == city_df.City_Id).where(col("City_Name") == "Lucknow").select("Trip_Id","Date","Passenger_Type","Distance_Travelled","Fare_Amount","Passenger_Rating","Driver_Rating")
    return df
